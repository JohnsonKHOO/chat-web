<template>
  <div class="grouplist_container">
    ...
    <!--群组信息列表-->
    <el-main>
      <el-table :data="tableData.filter(data => !search || data.user.account.toLowerCase().includes(search.toLowerCase())
        || data.name.toLowerCase().includes(search.toLowerCase()))" highlight-current-row border style="width: 100%;"
        class="grouplist_table" height="500">

        <el-table-column prop="id" label="群组id"></el-table-column>
        <el-table-column label="头像">
          <template v-slot="scope">
            <el-image v-if="scope.row.avatar" :src="require(
        ...
        <el-table-column prop="ownerId" label="群主id"></el-table-column>
        <el-table-column prop="user.account" label="群主账号"></el-table-column>
        ...
        <el-table-column label="操作" width="180" align="center">
          ...
          <template slot-scope="scope">
            <el-button type="text" icon="el-icon-edit" @click="listGroupMember(scope.row.id)">查看</el-button>
      ...
      <el-dialog title="新增群组信息" :visible.sync="addFormVisible" width="30%" close-on-click-modal="false"
        ...
        <el-form label-width="100px" :model="addForm" :rules="rules" ref="addForm" class="add_Form">
          ...
          <el-form-item label="群主id" prop="ownerId">
            <el-input v-model="addForm.ownerId" placeholder="请输入群组id" clearable></el-input>
          </el-form-item>
      ...
      <el-dialog title="编辑群组信息" :visible.sync="editFormVisible" width="30%" close-on-click-modal="false"
        ...
        <el-form label-width="100px" :model="editForm" :rules="rules" ref="editForm" class="edit_Form">
          <!--群名字输入框-->
          <el-form-item label="群名字：" prop="nickname">
            <el-input v-model="editForm.name" placeholder="请输入您的昵称" clearable></el-input>
          </el-form-item>
          <!--群主账号输入框-->
          <el-form-item label="群主id" prop="ownerId">
            <el-input v-model="editForm.ownerId" placeholder="请输入您的昵称" clearable></el-input>
          </el-form-item>
      ...
      <!--群组信息对话框-->  <!--未完成-->
      <el-dialog title="群组信息" :visible.sync="groupMemberVisible" width="30%" close-on-click-modal="false"
        close-on-press-escape="false" show-close="false">
        <el-table :data="groupMember.filter(data => !search || data.user.account.toLowerCase().includes(search.toLowerCase())
        || data.name.toLowerCase().includes(search.toLowerCase()))" highlight-current-row border style="width: 100%;"
          class="grouplist_table" height="500">
          ...
          <div slot="footer" class="dialog_footer">
            <el-button @click="groupMemberVisible = false">取消</el-button>
          </div>
      </el-dialog>
    ...
    methods: {
      ...
      SubmitEditForm() {
        ...
        this.$refs['editForm'].validate(valid => {
          ...
          if (valid) {
            var fd = new FormData();
            fd.append('group', JSON.stringify(this.editForm));
            this.$axios
              .post('/group/update', fd)
              .then(res => {
                console.log(res.data)
                if (res.data.code === 200) {
      ...
      listGroupMember(id) {
        ...
        fd.append('account', this.search);
        this.$axios
          .post('/group/user/listmember', fd)
          .then(res => {
            if (res.data.code === 200) {
      ...
      //还没做好
      groupMember(id) { //根据groupid查看按钮方法
        console.log(id);
        this.listGroupMember(id)
